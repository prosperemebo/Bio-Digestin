<template>
  <div
    class="h-full w-full bg-light p-6 flex justify-center items-center"
    v-if="pending"
  >
    <p>{{ loadingMessage }}</p>
  </div>
  ...
  <template v-else>
    ...
    <SignUpForm
      ...
      :newsletterCheckboxLabel="
        data?.signup_form_section.newsletter_checkbox_label
      "
      :promotionalCheckboxLabel="
        data?.signup_form_section.promotional_checkbox_label
      "
...
const loadingMessage = computed(() =>
  i18n.locale.value === "de"
    ? "Diese Seite wird geladen. Bitte haben Sie einen Moment Geduld..."
    : "This page is loading. Please hold on a moment..."
);

const errorMessage = computed(() =>
  i18n.locale.value === "de"
    ? "Beim Laden dieser Seite ist ein Fehler aufgetreten. Bitte versuchen Sie es später erneut."
